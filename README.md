...
- Node reserve is the disk cap for the machine. Admins can also set a per-user GB cap in Settings → Users (blank = node default). Uploads fail if either cap would be exceeded. Settings → Storage → Network connects Backblaze B2 (or R2 / MinIO / AWS / another Storebase). Same drive; fill order is yours — put Backblaze above this disk and new files go to B2 until that cap is full, then the next store. For B2: paste Bucket name + Endpoint from the bucket card, then keyID + applicationKey from App Keys → Your Application Keys (not Master Application Key). If you only have a keyID, create a new Application Key and copy the secret immediately. Region is inferred from `s3.<region>.backblazeb2.com`. The other node pastes this node’s inbound token. Secrets live in `data/network.json`; inbound blobs in `data/network-store/`.
- Passwords are scrypt hashes in `data/users.json`. Sessions are httpOnly cookies.
- Hidden `.trash`, `.temp`, `.versions`, `.temp-index.json`, `.trash-index.json`, `.storebase-meta.json`, and `.storebase-virus.json` live in that user’s folder.
...
- The Mac app (Settings → Mac app) pairs with a link + code, watches Downloads, and can route installers to Temp. It notifies you if the node is out of storage. The file list on the site updates live (SSE plus a 30s fallback poll, no refresh) when Mac uploads or anything else writes the drive. Cloud copies in Downloads keep the original name and icon, tagged Storebase in Finder; double-click always opens Storebase (then Preview/etc). Close deletes the local bytes. Delete on the Mac can trash the node copy; delete on the site removes the Mac copy too.
...
- Settings → Platform can install ClamAV and let users opt into upload scanning, force scanning on for the node, or disable it globally. Scan ratings follow moves, copies, ZIP extraction, user shares, and public links. Package installation runs as the Storebase service user; Linux nodes that are not running as root need passwordless `sudo` for the fixed package-manager command, or an admin can install the `clamav` package directly on the server.
...
The bottom-right operation panel stacks uploads, extraction, copies, moves, deletes, restores, and edits. You can keep browsing and start another operation while work runs. Uploads show transfer progress; extraction reports its phase and files written. Other operations show an activity bar until the server confirms success or failure. Successful entries dismiss after eight seconds; failed entries stay until dismissed. Progress is scoped to the current browser session, not a persistent job history.

Batches run up to three requests at once (two for extraction). Listings use bounded parallel I/O, previews request only the first 1.5 MB of text, and offscreen thumbnails load lazily. Live events refresh the file list without replacing it with a loading screen; the fallback poll and storage usage refresh run every 30 seconds. Settings and Terminal load on demand.

ZIP extraction checks capacity before inflation and decompresses asynchronously, then writes up to 16 files concurrently. Archives and their expanded contents still use memory, so very large archives remain limited by the node's available RAM. Concurrent writes reserve quota, copies/extractions reserve distinct destination names, and metadata updates use locks and atomic replacement.

Validation: `cd server && npm test && npm run check`; `cd app && npm run build && npm run lint`.
