...
- The Mac app (Settings → Mac app) pairs with a link + code, watches Downloads, and can route installers to Temp. It notifies you if the node is out of storage. The file list on the site updates live (no refresh) when Mac uploads or anything else writes the drive. Cloud copies in Downloads open in Preview/etc.; deleting one can trash it on the node too.
