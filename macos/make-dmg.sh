...
ARCH="$(uname -m)"
echo "Building Storebase 1.17 for ${ARCH}…"
...
if [[ "$INSTALL" == "1" ]]; then
  ...
  echo "Installed version: $(defaults read /Applications/Storebase.app/Contents/Info CFBundleShortVersionString 2>/dev/null || echo unknown)"
fi

echo
echo "Storebase 1.17 → $DMG"
echo "You should see a cube icon, a Dock icon, and a window that says 1.17."
